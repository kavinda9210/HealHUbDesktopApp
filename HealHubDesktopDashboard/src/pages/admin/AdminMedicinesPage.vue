...
async function load() {
  ...
  error.value = null
  try {
    const res = await api.get<{ success: boolean; data: MedicineRow[] }>('/api/medicines/medicines', {
...
async function applyFilters() {
  ...
  hasActiveFilters.value = !!(filters.value.q || filters.value.category || filters.value.status)
  await load()
}
...
function consumeNotice() {
  ...
  if (notice === 'medicine_created') showSuccess('Medicine created successfully.')
  else if (notice === 'medicine_updated') showSuccess('Medicine updated successfully.')
  else if (notice === 'medicine_deleted') showSuccess('Medicine deleted successfully.')
  const nextQuery: LocationQueryRaw = { ...route.query }
  delete (nextQuery as any).notice
  router.replace({ query: nextQuery })
}

function confirmDelete(medicineId: number) {
  pendingDeleteId.value = medicineId
...
async function executeDelete() {
  ...
  success.value = null
  try {
    await api.del(`/api/medicines/medicines/${id}`, { token: auth.accessToken })
    showSuccess('Medicine deleted successfully.')
    await load()
  } catch (e) {
    showError(e instanceof ApiError ? e.message : 'Failed to delete medicine')
  } finally {
    deletingId.value = null
  }
}

const isLowStock = (medicine: MedicineRow) => medicine.quantity_in_stock <= medicine.min_quantity
const formatPrice = (price?: number) => price != null ? `LKR ${price.toFixed(2)}` : '—'
  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-heading">Medicines</h1>
        <p class="page-sub">{{ isLoading ? 'Loading…' : `${rows.length} medicine${rows.length !== 1 ? 's' : ''} available` }}</p>
      </div>
      <div class="header-actions">
        ...
        <router-link class="btn-primary" to="/admin/medicines/create">
    ...
    <div class="filters-section">
      <div class="filters-header">
        ...
        <span v-if="hasActiveFilters" class="filter-badge">{{ hasActiveFilters ? '✓ Filters Active' : '' }}</span>
      </div>
        ...
        <div class="filters-grid">
          <div class="filter-field">
            ...
            <div class="search-input">
              ...
              <input v-model="filters.q" type="text" placeholder="Search by name or generic name…" />
            </div>
          </div>
          ...
          <div class="filter-actions">
            ...
            <button v-if="hasActiveFilters" type="button" class="btn-secondary btn-sm" @click="clearFilters">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16">
    ...
    <div class="table-card">
      <div class="table-header">
        <h3 class="table-title">Medicines Catalog</h3>
        <span class="table-count">{{ rows.length }} item{{ rows.length !== 1 ? 's' : '' }}</span>
      </div>
      <div class="table-wrapper">
        <table class="data-table" role="grid">
          ...
          <tbody v-if="!isLoading && rows.length > 0">
            <tr v-for="medicine in rows" :key="medicine.medicine_id" class="table-row">
              <td class="col-name"><strong>{{ medicine.medicine_name }}</strong></td>
              <td class="col-generic">{{ medicine.generic_name || '—' }}</td>
              <td class="col-category"><span class="badge badge-info">{{ medicine.category || '—' }}</span></td>
              <td class="col-strength">{{ medicine.strength || '—' }}{{ medicine.strength && medicine.unit ? ' ' + medicine.unit : '' }}</td>
              <td class="col-price">{{ formatPrice(medicine.unit_price) }}</td>
              <td class="col-stock">
                <div class="stock-indicator" :class="{ 'stock-low': isLowStock(medicine), 'stock-ok': !isLowStock(medicine) }">
                  <strong>{{ medicine.quantity_in_stock }}</strong>
                  <span v-if="isLowStock(medicine)" class="stock-label">Low</span>
                </div>
              </td>
              <td class="col-status">
                <span class="status-badge" :class="`status-${medicine.status.toLowerCase()}`">
                  {{ medicine.status }}
                </span>
              </td>
              <td class="col-actions">
                <div class="action-buttons">
                  <router-link :to="`/admin/medicines/${medicine.medicine_id}/edit`" class="btn-action btn-edit" title="Edit">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16">
                  ...
                  <button
                    ...
                    @click="confirmDelete(medicine.medicine_id)"
                    title="Delete"
                  >
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16">
          ...
          <tbody v-else-if="!isLoading && rows.length === 0">
            <tr>
              <td colspan="8" class="empty-state">
                ...
                <p class="muted">Try adjusting your filters or <router-link to="/admin/medicines/create">add a new medicine</router-link></p>
              </td>
            </tr>
          </tbody>
          <tbody v-else>
            <tr>
              <td colspan="8" class="empty-state loading">
