  ...
  <div>
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <div class="text-sm text-gray-500">Update doctor information</div>
      </div>
      <div class="flex items-center gap-2">
        <router-link class="rounded border border-gray-300 px-3 py-2 text-sm" :to="`/admin/doctors/${route.params.doctorId}`">
          Cancel
        </router-link>
      </div>
    </div>
    ...
    <div v-else-if="isLoading" class="mt-6 rounded border border-gray-200 bg-white px-4 py-3 text-sm text-gray-600">
      Loading…
    </div>
    ...
    <form v-else class="mt-6 rounded border border-gray-200 bg-white p-4" @submit.prevent="save">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <input v-model="form.qualification" class="mt-1 w-full rounded border border-gray-300 px-3 py-2" />
        </div>
        <div>
        ...
        <div v-if="form.is_available" class="md:col-span-2">
          <label class="block text-xs font-medium text-gray-600">Available days</label>
          <div class="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <label v-for="d in daysOfWeek" :key="d" class="flex items-center gap-2 text-sm text-gray-700">
        ...
        <div v-if="form.is_available">
          ...
          <input v-model="form.start_time" type="time" class="mt-1 w-full rounded border border-gray-300 px-3 py-2" />
        </div>
        <div v-if="form.is_available">
