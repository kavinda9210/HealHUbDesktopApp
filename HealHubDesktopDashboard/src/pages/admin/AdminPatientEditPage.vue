...
async function load() {
  if (!Number.isFinite(patientId.value)) { loadError.value = 'Invalid patient id'; return }
  ...
  try {
    ...
    loadError.value = e instanceof ApiError ? e.message : 'Failed to load patient'
  } finally {
  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-left">
        <router-link class="back-link" :to="`/admin/patients/${route.params.patientId}`">
        ...
        <p class="page-sub">Update the patient's profile and medical information.</p>
      </div>
    </div>
    ...
    <form v-else class="form-layout" @submit.prevent="save" novalidate>
      ...
      <div class="form-section">
        <div class="section-head">
          ...
          <div>
            ...
            <div class="section-sub">Login email for the patient</div>
          </div>
        </div>
      ...
      <div class="form-actions">
        <router-link class="btn-ghost" :to="`/admin/patients/${route.params.patientId}`">Cancel</router-link>
...
.state-spinner { color: var(--g-500, #2E8B57); animation: spin .85s linear infinite; }

/* ════════════════════════════════════════════════
   FORM
   ════════════════════════════════════════════════ */
.form-layout { display: flex; flex-direction: column; gap: 1.125rem; }
...
.ftextarea { resize: vertical; min-height: 90px; line-height: 1.55; }
