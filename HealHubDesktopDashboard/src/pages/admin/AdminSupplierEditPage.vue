...
async function loadSupplier() {
  ...
  try {
    ...
    loadError.value = e instanceof ApiError ? e.message : 'Failed to load supplier'
  } finally {
...
async function updateSupplier() {
  ...
  try {
    ...
    await api.put(`/api/medicines/suppliers/${supplierId}`, payload, { token: auth.accessToken })
    router.replace({ path: '/admin/suppliers', query: { notice: 'supplier_updated' } })
  } catch (e) {
    createError.value = e instanceof ApiError ? e.message : 'Failed to update supplier'
  } finally {
...
onMounted(() => {
  if (supplierId) {
    ...
    loadError.value = 'Invalid supplier ID'
  }
})
</script>
  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <h1 class="page-heading">Edit Supplier</h1>
        <p class="page-sub">Update the supplier details below.</p>
      </div>
    </div>
    ...
    <!-- ── Form ────────────────────────────────── -->
    <form v-if="!isLoading" class="form-layout" @submit.prevent="updateSupplier" novalidate>
