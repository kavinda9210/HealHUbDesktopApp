...
async function createMedicine() {
  ...
  try {
    ...
    const qtyRaw = String(form.value.quantity_in_stock ?? '').trim()
    const minRaw = String(form.value.min_quantity ?? '').trim()
    const maxRaw = String(form.value.max_quantity ?? '').trim()
    const priceRaw = String(form.value.unit_price ?? '').trim()

    const qtyInStock = qtyRaw ? Number(qtyRaw) : 0
    const minQty = minRaw ? Number(minRaw) : 10
    const maxQty = maxRaw ? Number(maxRaw) : 500
    ...
    console.info('Creating medicine payload:', payload)
    console.info('Access token present:', Boolean(auth.accessToken))
    await api.post('/api/medicines/medicines', payload, { token: auth.accessToken })
    router.replace({ path: '/admin/medicines', query: { notice: 'medicine_created' } })
  } catch (e) {
    console.error('Failed to create medicine (client):', e)
    createError.value = e instanceof ApiError ? e.message : 'Failed to create medicine'
  } finally {
  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <h1 class="page-heading">Add New Medicine</h1>
        <p class="page-sub">Fill in the details below to add a new medicine to the catalog.</p>
      </div>
    </div>

    <form class="form-layout" @submit.prevent="createMedicine" novalidate>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn-primary" :disabled="createLoading">
          ...
          {{ createLoading ? 'Creating...' : 'Create Medicine' }}
        </button>
      </div>
