  ...
  <div>
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <div class="text-sm text-gray-500">Manage patients</div>
      </div>
        ...
        <table class="min-w-full text-left text-sm">
          ...
          <tbody>
            <tr v-for="r in rows" :key="r.patient_id" class="border-t border-gray-100">
              ...
              <td class="px-4 py-2 text-right">
                <div class="flex justify-end gap-2">
                  <router-link class="rounded border border-gray-300 px-3 py-1.5 text-xs" :to="`/admin/patients/${r.patient_id}`">
                    View
                  </router-link>
                  <router-link class="rounded border border-gray-300 px-3 py-1.5 text-xs" :to="`/admin/patients/${r.patient_id}/edit`">
                    Edit
                  </router-link>
                  <router-link class="rounded border border-gray-300 px-3 py-1.5 text-xs" :to="`/admin/patients/${r.patient_id}/alerts`">
                    Alerts
                  </router-link>
                  <button class="rounded border border-gray-300 px-3 py-1.5 text-xs" @click="deletePatient(r.patient_id)">
                    Delete
                  </button>
                </div>
              </td>
            ...
            <tr v-if="!isLoading && rows.length === 0">
              <td class="px-4 py-4 text-sm text-gray-600" colspan="6">No patients found.</td>
            </tr>
          </tbody>
        </table>
      </div>
