...
async function deleteAmbulance(ambulanceId: number) {
  if (!confirm('Delete this ambulance staff? This action cannot be undone.')) return
  deleteLoadingId.value = ambulanceId
  ...
  <div class="page">
    ...
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-heading">Ambulances</h1>
        <p class="page-sub">View, search and manage all ambulance staff.</p>
      </div>
      <router-link class="btn-primary" to="/admin/ambulances/create">
    ...
    <div class="form-section">
      <div class="section-head">
        ...
        <div>
          <div class="section-title">Filter Ambulance Staff</div>
          <div class="section-sub">Search by driver name or ambulance number</div>
        </div>
      </div>
        ...
        <div class="field filter-field">
          ...
          <div class="input-wrap">
            ...
            <input id="filter-q" v-model="filters.q"
              class="finput finput--icon" placeholder="Search by driver name…" />
          </div>
        </div>
    ...
    <div class="form-section table-section">
      ...
      <div class="table-header">
        <div class="table-header-left">
          ...
          <div>
            <div class="section-title">Ambulance Staff List</div>
            <div class="section-sub">
              {{ isLoading ? 'Loading…' : `${rows.length} unit${rows.length !== 1 ? 's' : ''}` }}
            </div>
          </div>
        </div>
      ...
      <div class="table-wrap">
        <table class="ptable">
          ...
          <tbody>
            <!-- skeleton -->
            <tr v-if="isLoading" v-for="i in 4" :key="'sk-'+i" class="ptable-row">
              <td><div class="driver-cell"><span class="skeleton-avatar"></span><span class="skeleton skeleton--md"></span></div></td>
            ...
            <template v-if="!isLoading">
              <tr v-for="r in rows" :key="r.ambulance_id" class="ptable-row">
                <!-- Driver -->
                <td>
                  <div class="driver-cell">
                    ...
                    <span class="driver-name">{{ r.driver_name || '—' }}</span>
                  </div>
                </td>

                <!-- Ambulance number -->
                <td>
                  <span v-if="r.ambulance_number" class="amb-number">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"
                ...
                <td class="cell-muted">{{ r.driver_phone || '—' }}</td>
                <td class="cell-muted">{{ r.email || '—' }}</td>

                <!-- Availability chip -->
                <td>
                  <span class="avail-chip" :class="r.is_available ? 'chip--on' : 'chip--off'">
                ...
                <td>
                  <div class="row-actions">
                    <router-link class="action-btn action-btn--view" :to="`/admin/ambulances/${r.ambulance_id}`">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"
                        stroke-linecap="round" stroke-linejoin="round" width="13" height="13">
                    ...
                    <router-link class="action-btn action-btn--edit" :to="`/admin/ambulances/${r.ambulance_id}/edit`">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"
                        stroke-linecap="round" stroke-linejoin="round" width="13" height="13">
                    ...
                    <router-link class="action-btn action-btn--alerts" :to="`/admin/ambulances/${r.ambulance_id}/alerts`">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75"
                        stroke-linecap="round" stroke-linejoin="round" width="13" height="13">
                    ...
                    <button class="action-btn action-btn--delete"
                      :disabled="deleteLoadingId === r.ambulance_id"
                      @click="deleteAmbulance(r.ambulance_id)">
                      <svg v-if="deleteLoadingId === r.ambulance_id"
              ...
              <tr v-if="rows.length === 0">
                <td colspan="6">
                  <div class="empty-state">
                    ...
                    <p class="empty-title">No ambulance staff found</p>
                    <p class="empty-sub">Try adjusting your search or add a new unit.</p>
                    <router-link class="btn-primary btn-sm" style="margin-top:.75rem" to="/admin/ambulances/create">
                      Add Ambulance Staff
                    </router-link>
                  </div>
                </td>
...
/* driver cell */
.driver-cell { display: flex; align-items: center; gap: .625rem; }
.driver-avatar {
...
:global(.shell[data-theme="dark"]) .driver-avatar { background: #2c1212; border-color: #6b2020; color: #f87171; }
.driver-name { font-weight: 700; color: var(--text-1); }
...
.amb-number {
  ...
  color: var(--text-2); font-family: monospace;
  letter-spacing: .03em;
}
...
.chip--on  { background: #f0faf5; color: #1e6b41; border: 1px solid rgba(46,139,87,.2); }
.chip--off { background: #fdf4f4; color: #9b1d1d; border: 1px solid rgba(200,40,40,.12); }
.chip-dot  { width: 6px; height: 6px; border-radius: 50%; }
.chip--on  .chip-dot { background: var(--g-500, #2E8B57); }
.chip--off .chip-dot { background: #dc2626; }

:global(.shell[data-theme="dark"]) .chip--on  { background: #1a3327; color: #6cd49a; border-color: #2a5540; }
:global(.shell[data-theme="dark"]) .chip--off { background: #2c1212; color: #e08585; border-color: #6b2020; }
...
.skeleton {
  display: inline-block; border-radius: 6px; height: 13px;
  background: linear-gradient(90deg, var(--border) 25%, var(--bg-page) 50%, var(--border) 75%);
  background-size: 200% 100%; animation: shimmer 1.4s infinite;
}
.skeleton--xs { width: 60px; }
...
.action-btn--view   { background: var(--bg-page); border-color: var(--border); color: var(--text-3); }
